...
def get_tiles(ds, tile_rows=256, tile_cols=256):
...
def run_prediction_orthophoto(args):
    ...
    # sicherstellen, dass die Tiles zum Modell passen
    args.img_rows = int(args.img_rows)
    args.img_cols = int(args.img_cols)
    ...
    PRED_BATCH = 4  # oder 2 oder 1, je nach VRAM

    X_test_prepro = X_test_prepro.astype('float16')  # wenn stabil, sonst float32
    n_tiles = X_test_prepro.shape[0]
    preds_list = []

    for start in range(0, n_tiles, PRED_BATCH):
        end = min(start + PRED_BATCH, n_tiles)
        batch = X_test_prepro[start:end]
        batch_pred = model.predict(batch, batch_size=PRED_BATCH, verbose=0)
