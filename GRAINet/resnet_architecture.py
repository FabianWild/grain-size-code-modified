...
def initialize_weights(model, layer_name=None):
    # Unter TF2 macht Keras die Initialisierung selbst beim ersten Aufruf.
    # Wir triggern höchstens einen Dummy-Forward-Pass, damit alle Weights angelegt sind.
    try:
        if tf.executing_eagerly():
            # Dummy forward to build weights if not built yet
            input_shape = model.input_shape  # (None, H, W, C)
            if None not in input_shape:
                import numpy as np
                dummy = np.zeros((1, input_shape[1], input_shape[2], input_shape[3]), dtype="float32")
