...
Image.MAX_IMAGE_PIXELS = None          # Sicherheitsgrenze deaktivieren
ImageFile.LOAD_TRUNCATED_IMAGES = True # (optional) toleranter laden
...
def estimate_siso_simo_1image(vars, image, greyscale,
    ...
    """
    # --- Normalisierung vor Multi-Model-Block (direkt vor if type(BATCH_SIZE)==list:) ---
# Falls VALID_BATCH_SIZE nicht gesetzt ist, nimm BATCH_SIZE als Default
    try:
        VALID_BATCH_SIZE
    except NameError:
        VALID_BATCH_SIZE = BATCH_SIZE

    # Wenn BATCH_SIZE eine Liste ist, aber weights_path nur ein String → repliziere denselben Pfad
    if isinstance(BATCH_SIZE, list) and isinstance(weights_path, str):
        weights_path = [weights_path] * len(BATCH_SIZE)

    # Wenn BATCH_SIZE eine Liste ist, aber VALID_BATCH_SIZE kein Liste → repliziere auf gleiche Länge
    if isinstance(BATCH_SIZE, list) and not isinstance(VALID_BATCH_SIZE, list):
        VALID_BATCH_SIZE = [VALID_BATCH_SIZE] * len(BATCH_SIZE)
    # --- Ende Normalisierung ---
    ...
    if type(BATCH_SIZE) == list:
        ...
        for batch_size, valid_batch_size, wp in zip(BATCH_SIZE, VALID_BATCH_SIZE, weights_path):
            ...
            # --- Prüfen, ob das Gewicht existiert, sonst im Arbeitsverzeichnis suchen ---
            import os
            if not os.path.exists(wp):
                ...
                for root, dirs, files in os.walk(os.getcwd()):
                    if base in files:
                        ...
                        break
                if found:
                    print(f"[INFO] Using found weight file: {found}")
                    wp = found
                else:
                    raise FileNotFoundError(f"⚠️ Gewichtedatei nicht gefunden: {wp}")
            # --------------------------------------------------------------------------
    ...
    if scale==True:
        # 'weights_path' kann String oder Liste sein -> nimm erstes Element
        wp0 = weights_path[0] if isinstance(weights_path, list) else weights_path
        CS = joblib.load(wp0.replace('.hdf5','_scaler.pkl'))
    else:
        CS = []
    ...
    def _first_float(x):
        a = np.array(x, dtype='float32').ravel()
        return float(a[0])

    # ...
    if isinstance(result, list):
        result = [_first_float(r) for r in result]
    else:
        result = [_first_float(result)]
    ...
    if type(SM) == list:
       ...
       wp0 = weights_path[0] if isinstance(weights_path, list) else weights_path
       Z = joblib.load(wp0.replace('.hdf5','_bias.pkl'))
